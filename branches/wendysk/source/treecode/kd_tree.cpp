...
void ANNkd_leaf::getAc(ANNorthRect &/*bnd_box*/,vector<double> const& qpoint,
	double /*openangle*/,vector<double> &res,int dim)
{
	double dist2=0;
	for(int i=0;i<dim;++i)
	{
		dist2+=(RCM[i]-qpoint[i])*(RCM[i]-qpoint[i]);
	}
	dist2=sqrt(dist2);
	if(dist2>h)
	{
		for(int i=0;i<dim;++i)
		{
			res[i]+=TotalMass*(RCM[i]-qpoint[i])/(dist2*dist2*dist2);
		}
	}
	else
	{
		double dist3=1/(h*h*h);
		for(int i=0;i<dim;++i)
		{
			res[i]+=TotalMass*(RCM[i]-qpoint[i])*dist3;
		}
	}
	return;
}
...
void ANNkd_split::getAc(ANNorthRect &bnd_box,vector<double> const& qpoint,
	double openangle,vector<double> &res,int dim)
{
	double size=bnd_box.hi[0]-bnd_box.lo[0];
	double temp;
	for(int i=1;i<dim;++i)
	{
		temp=bnd_box.hi[i]-bnd_box.lo[i];
		if(temp>size)
			size=temp;
	}
	double dist2=0;
	vector<double>::iterator itr=RCM.begin();
	vector<double>::const_iterator itq=qpoint.begin();
	for(;itr!=RCM.end();++itr,++itq)
	{
		dist2+=(*itr-*itq)*(*itr-*itq);
	}
	dist2=sqrt(dist2);
	if(size<openangle*dist2)
	{
		double dist3=1/(dist2*dist2*dist2);
		vector<double>::iterator itres=res.begin();
		itq=qpoint.begin();
		for(itr=RCM.begin();itr!=RCM.end();++itr,++itq,++itres)
		{
			*itres+=TotalMass*(*itr-*itq)*dist3;
		}
		return;
	}
	else
	{
		double temp=bnd_box.hi[cut_dim];
		bnd_box.hi[cut_dim]=cut_val;
		child[0]->getAc(bnd_box,qpoint,openangle,res,dim);
		bnd_box.hi[cut_dim]=temp;
		bnd_box.lo[cut_dim]=cut_val;
		child[1]->getAc(bnd_box,qpoint,openangle,res,dim);
		return;
	}
}
...
vector<double> ANNkd_tree::GetAcc(std::vector<double> const& qpoint,
	double openAngle)
{
	std::vector<double> res(qpoint.size(),0);
	ANNorthRect bnd_box(dim, bnd_box_lo, bnd_box_hi);
	root->getAc(bnd_box,qpoint,openAngle,res,dim);
	return res;
}
	...
	ANNsplitRule		split)			// splitting method
{
	...
	switch (split) {					// build by rule
	case ANN_KD_STD:					// standard kd-splitting rule
		//root = rkd_tree(masses,pa, pidx, n, dd, bs, bnd_box, kd_split);
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, kd_split);
		break;
	case ANN_KD_MIDPT:					// midpoint split
		//root = rkd_tree(masses,pa, pidx, n, dd, bs, bnd_box, midpt_split);
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, midpt_split);
		break;
	case ANN_KD_FAIR:					// fair split
		//root = rkd_tree(masses,pa, pidx, n, dd, bs, bnd_box, fair_split);
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, fair_split);
		break;
	case ANN_KD_SUGGEST:				// best (in our opinion)
	case ANN_KD_SL_MIDPT:				// sliding midpoint split
		//root = rkd_tree(masses,pa, pidx, n, dd, bs, bnd_box, sl_midpt_split);
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, sl_midpt_split);
		break;
	case ANN_KD_SL_FAIR:				// sliding fair split
		//root = rkd_tree(masses,pa, pidx, n, dd, bs, bnd_box, sl_fair_split);
		root = rkd_tree(pa, pidx, n, dd, bs, bnd_box, sl_fair_split);
		break;
	default:
		annError("Illegal splitting method", ANNabort);
...
{
	double size=cd_bnds[1]-cd_bnds[0];
	double dist2=0;
	vector<double>::iterator itr=RCM.begin();
	vector<double>::const_iterator itq=qpoint.begin();
	for(;itr!=RCM.end();++itr,++itq)
	...
	{
		child[0]->getPot(qpoint,openangle,soft,res);
		child[1]->getPot(qpoint,openangle,soft,res);
		return;
	}
}

void ANNkd_leaf::getPot(vector<double> const& qpoint,
	double /*openangle*/,double soft,double &res)
{
	double dist2=0;
	vector<double>::iterator itr=RCM.begin();
...
double ANNkd_tree::GetPot(std::vector<double> const& qpoint,double openAngle,
	double soft)
{
	double res=0;
...
void ANNkd_split::SetH(vector<double> const& Hvec)
{
	child[0]->SetH(Hvec);
	child[1]->SetH(Hvec);
}

void ANNkd_leaf::SetH(vector<double> const& Hvec)
{
	h=Hvec[bkt[0]];
}

void ANNkd_leaf::SetCmax(vector<double> const& Cmax)
{
	cmax=Cmax[bkt[0]];
}

void ANNkd_leaf::SetVmax(vector<double> const& Vmax)
{
	vmax=Vmax[bkt[0]];
}

void ANNkd_leaf::SetMass(vector<double> const& masses,int /*dim*/)
{
	TotalMass=masses[bkt[0]];
}

void ANNkd_split::SetCmax(vector<double> const& Cmax)
{
	child[0]->SetCmax(Cmax);
	child[1]->SetCmax(Cmax);
	cmax=max(child[0]->GetCmax(),child[1]->GetCmax());
}

void ANNkd_split::SetVmax(vector<double> const& Vmax)
{
	child[0]->SetVmax(Vmax);
	child[1]->SetVmax(Vmax);
	vmax=max(child[0]->GetVmax(),child[1]->GetVmax());
}

void ANNkd_split::SetMass(vector<double> const& masses,int dim)
{
	RCM.resize(dim);
	child[0]->SetMass(masses,dim);
	child[1]->SetMass(masses,dim);
	TotalMass=child[0]->GetMass()+child[1]->GetMass();
	vector<double>const *rcm0=child[0]->GetRCM();
	vector<double>const *rcm1=child[1]->GetRCM();
	double Mass_1=1/TotalMass;
	for(int i=0;i<dim;++i)
	{
		RCM[i]=rcm0->at(i)*child[0]->GetMass()+
			rcm1->at(i)*child[1]->GetMass();
		RCM[i]*=Mass_1;
	}
}

void ANNkd_tree::SetMass(vector<double> const& masses)
{
	root->SetMass(masses,dim);
}

void ANNkd_tree::SetCmax(vector<double> const&Cmax)
{
	root->SetCmax(Cmax);
}

void ANNkd_tree::SetVmax(vector<double> const& Vmax)
{
	root->SetVmax(Vmax);
}

double ANNkd_tree::GetTimeStep(int index,vector<vector<double> >const& vel
	,double dt,vector<double> const& c)
{
	ANNorthRect bnd_box(dim,bnd_box_lo,bnd_box_hi);
	double v_i=0;
	for(int i=0;i<dim;++i)
		v_i+=vel[index][i]*vel[index][i];
	return root->GetTimeStep(index,vel,bnd_box,pts,c,dt,sqrt(v_i));
}

double ANNkd_leaf::GetTimeStep(int index,vector<vector<double> >
	const& vel,ANNorthRect &/*bnd_box*/,ANNpointArray pa,
	vector<double> const& c,double dt,double /*v_i*/)const
{
	int dim=vel[0].size();
	double r_ij=0,scalprod=0;
	for(int i=0;i<dim;++i)
		r_ij+=(pa[bkt[0]][i]-pa[index][i])*(pa[bkt[0]][i]-pa[index][i]);
	for(int i=0;i<dim;++i)
		scalprod+=(vel[bkt[0]][i]-vel[index][i])*(pa[bkt[0]][i]-pa[index][i]);
	double res=abs(r_ij/((cmax+c[index])*sqrt(r_ij)-scalprod));
	if(res<dt)
		return res;
	else
		return dt;
...
double ANNkd_split::GetTimeStep(int index,vector<vector<double> >
	const& vel,ANNorthRect &bnd_box,ANNpointArray pa,
	vector<double> const& c,double dt,double v_i)const
{
	int dim=vel[0].size();
	double d=bnd_box.hi[0]-pa[index][0];
	for(int i=1;i<dim;++i)
		if(bnd_box.hi[i]-pa[index][i]<d)
			d=bnd_box.hi[i]-pa[index][i];
	for(int i=0;i<dim;++i)
		if(pa[index][i]-bnd_box.lo[i]<d)
			d=pa[index][i]-bnd_box.lo[i];

	if(d<dt*(c[index]+cmax+vmax+v_i))
	{
		double hv = bnd_box.hi[cut_dim];			// save box bounds
		bnd_box.hi[cut_dim] = cut_val;				// upper bound for low child
