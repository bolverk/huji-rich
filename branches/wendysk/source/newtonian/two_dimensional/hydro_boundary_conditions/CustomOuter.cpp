...
vector<double> CustomOuter::CalcTracerFlux(Tessellation const* tessellation,
	...
	double time,ScalarInterpolation const* interp) const
{
	...
	if(IsGhostCell(edge.GetNeighbor(0),tessellation))
		...
		angle=atan2(n.y,n.x);
	if(angle>-0.25*M_PI&&angle<0.25*M_PI)
		return _right->CalcTracerFlux(tessellation,tracers,dm,edge,index,
		dt,time,interp);
	if(angle<0.75*M_PI&&angle>0.25*M_PI)
		return _up->CalcTracerFlux(tessellation,tracers,dm,edge,index,
		dt,time,interp);
	if(abs(angle)>0.75*M_PI)
		return _left->CalcTracerFlux(tessellation,tracers,dm,edge,index,
		dt,time,interp);
	if(angle>-0.75*M_PI&&angle-0.25*M_PI)
		return _down->CalcTracerFlux(tessellation,tracers,dm,edge,index,
		dt,time,interp);
	throw UniversalError("error in custom Outer conditions");
}
