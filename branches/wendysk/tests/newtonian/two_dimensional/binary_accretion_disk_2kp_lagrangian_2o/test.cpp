...
public:
  ...
  void setCustomEvolution(vector<CustomEvolution*>* cuev)
  {
    cuev_ = cuev;
  }
				     ...
				     double time)
  {
	...
	if(cuev_->at(i)){
	  ...
	  for(int j=0;j<(int)neighbor_list.size();++j){
	    if(neighbor_list[j]>=0 && neighbor_list[j]<(int)cuev_->size())
	      real_neighbors.push_back(neighbor_list[j]);
	  }
	  for(int j=0;j<(int)real_neighbors.size();++j){
	    if(cuev_->at(real_neighbors[j])){
	      result[i] = Vector2D(0,0);
	    }
	  }
	}
...
public:
  ...
  SimData(MagicNumbers const& mm = MagicNumbers()):
    ...
    pm_naive_(),
    pm_inter_(pm_naive_),
    point_motion_(mm.min_radius+2*mm.cell_size,
		  mm.max_radius-mm.cell_size,
		  pm_inter_),
    rs_(),
    ...
    sim_(grid_func(),  
	 ...
	 false)
  {
    point_motion_.setCustomEvolution(&sim_.CellsEvolve);
...
int main(void)
{
  ...
  write_number("nr_inner.txt",mm.nr_inner);
  write_number("nr_outer.txt",mm.nr_outer);
