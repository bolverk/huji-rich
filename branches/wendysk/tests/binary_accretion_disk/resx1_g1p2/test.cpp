...
public:
  ...
  MagicNumbers(void):
    lower_left(-0.5,-0.5),
    upper_right(0.5,0.5),
    total_point_number(2000),
    min_radius(0.03),
    max_radius(0.48),
    cell_size(sqrt(M_PI*(pow(max_radius,2)-pow(min_radius,2))/
		   total_point_number)),
    nr_inner(2*int(2*M_PI*min_radius/cell_size)),
    nr_outer(2*int(2*M_PI*max_radius/cell_size)),
    center(0,0),
    adiabatic_index(1.2),
    density(1e-5),
...
namespace{
			    ...
			    double volume_thres)
  {
	...
	else if(tess->GetVolume(i)<volume_thres)
	  res[i] = 1.0/tess->GetVolume(i);
      }
    }
    return res;
  }
  ...
  public:
	...
	if(abs(tess->GetMeshPoint(stage2[i]))<max_radius_-4*cell_size_)
	  stage3.push_back(stage2[i]);
      }
      vector<int> res;
      for(int i=0;i<(int)stage3.size();++i){
	if(tess->GetVolume(stage3[i])>a_min_)
  ...
  void my_main_loop(hdsim& sim)
  {
    ...
    ConsecutiveSnapshots diag(endtime/10000);
    try{
      /*
      main_loop(sim,
      term_cond,
	...
	(sim,
	 ...
	 0.5/mm.total_point_number/20.0);
...
public:
  ...
  SimData(MagicNumbers const& mm = MagicNumbers()):
    ...
    pm_naive_(),
    pm_inter_(pm_naive_),
    point_motion_(mm.min_radius+1.25*mm.cell_size,
		  mm.max_radius-1.25*mm.cell_size,
		  pm_inter_),
    rs_(),
