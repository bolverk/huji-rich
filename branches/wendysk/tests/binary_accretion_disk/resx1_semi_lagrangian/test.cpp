...
class RestricAzimuthal: public PointMotion
{
public:

  RestricAzimuthal(PointMotion& naive):
    naive_(naive) {}
  ...
  vector<Vector2D> calcAllVelocities(Tessellation const* tess,
				     ...
				     double time)
  {
    vector<Vector2D> result = naive_.calcAllVelocities(tess,
						       cells,
						       time);
    for(int i=0;i<(int)tess->GetPointNo();++i){
      const Vector2D point = tess->GetMeshPoint(i);
      const Vector2D rhat = point/abs(point);
      result[i] = result[i] - rhat*ScalarProd(result[i],rhat);
    }
    return result;
  }
...
private:
  ...
  RestricAzimuthal point_motion_;
  Eulerian eulerian_;
  const Hllc rs_;
