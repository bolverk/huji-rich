...
{
public:
	virtual std::set<Vector3D> operator()(const Delaunay &del, const OuterBoundary3D &boundary) const = 0;
};
...
{
public:
	virtual std::set<Vector3D> operator()(const Delaunay &del, const OuterBoundary3D &boundary) const;
};
...
private:
	...
	typedef map<Vector3D, unordered_set<Subcube>> breach_map;
