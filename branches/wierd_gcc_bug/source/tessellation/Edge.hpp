...
public:
  ...
  /*! \brief Returns the index of neighbor cell adjacent to the given cell, on the edge's other side
    \param cell original cell
    \return Cell index of adjacent cell
   */
  //  int get_other_adjacent_cell(int cell) const;

  //! \brief Returns the x-coordinate from one of the ends of the edge. \param index Which end of the edge to return. \returns The chosen x-coordinate.
	double get_x(int index)const{if(index==0) return _p1.get_x();
	else return _p2.get_x();}
	//! \brief Returns the y-coordinate from one of the ends of the edge. \param index Which end of the edge to return. \returns The chosen y-coordinate.
	double get_y(int index)const{if(index==0) return _p1.get_y();
	else return _p2.get_y();}
