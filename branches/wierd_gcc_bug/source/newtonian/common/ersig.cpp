...
/*
double ps_two_strong_shocks(Primitive const& left, 
			    Primitive const& right, 
			    double g)
{
  const double dl = left.Density;
  ...
  const double dr = right.Density;
  const double vr = right.Velocity.x;
  return (dl*dr*(1+g)*pow(vl-vr,2))/(2.*pow(sqrt(dl)+sqrt(dr),2));
}
*/

/*
double ps_acoustic_rp(Primitive const& left,
		      Primitive const& right,
		      double g)
{
  const double dl = left.Density;
  ...
  const double pr = right.Pressure;
  const double vr = right.Velocity.x;
  return (sqrt(dl*g*pl)*pr+sqrt(dr*g*pr)*(pl+sqrt(dl*g*pl)*(vl-vr)))/
    (sqrt(dl*g*pl)+sqrt(dr*g*pr));
}
*/
}
...
//! \brief Abstract class for spatial distribution of hydrodynamic variables
class HydroProf
{
public:

  /*! \brief Returns hydrodynamic variables
    \param v Dimensionless coordinate
