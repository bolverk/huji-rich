...
public:
  ...
  SimData(const Snapshot* const ss = nullptr):
    ...
    sim_(tess_,
	 outer_,
	 pg_,
	 ss ? ss->cells : calc_init_cond(tess_),
	 eos_,
	 point_motion_,
	 evc_,
	 force_,
...
int main(void)
{
...

  {
    ...
    my_main_loop(sim, 0.001);
    
    write_snapshot_to_hdf5(sim,"checkpoint.h5");
  }
  {
    SimData sim_data(new Snapshot(read_hdf5_snapshot("checkpoint.h5")));
    hdsim& sim = sim_data.getSim();

    my_main_loop(sim,0.003);
