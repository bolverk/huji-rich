...
bool PointInPoly2(Tessellation3D const& tess, Vector3D const& point, std::size_t index)
{
...
#ifdef RICH_MPI
vector<Vector3D> StaticVoronoi::UpdateMPIPoints(Tessellation3D const& vproc, int rank,
...
StaticVoronoi::StaticVoronoi()
{}

StaticVoronoi::StaticVoronoi(Vector3D const& ll, Vector3D const& ur) :ll_(ll), ur_(ur), Norg_(0) {}

void StaticVoronoi::CalcRigidCM(std::size_t face_index)
{
...
vector<Vector3D> StaticVoronoi::CreateBoundaryPoints(vector<std::pair<std::size_t, std::size_t> > const& to_duplicate)
{
...
#ifdef RICH_MPI
vector<Vector3D> StaticVoronoi::CreateBoundaryPointsMPI(vector<std::pair<std::size_t, std::size_t> > const& to_duplicate,
...
vector<vector<std::size_t> > const& StaticVoronoi::GetGhostIndeces(void) const
{
	return Nghost_;
}

#ifdef RICH_MPI
void StaticVoronoi::Build(vector<Vector3D> const & points, Tessellation3D const& tproc)
{
...
void StaticVoronoi::CalcAllCM(void)
{
...
std::pair<Vector3D, Vector3D> StaticVoronoi::GetBoxCoordinates(void)const
{
	return std::pair<Vector3D, Vector3D>(ll_, ur_);
}

void StaticVoronoi::BuildNoBox(vector<Vector3D> const& points, vector<Vector3D> const& ghosts, size_t toduplicate)
{
	assert(points.size() > 0);
	// Clear data
	PointTetras_.clear();
	...
	vector<Vector3D> extra_points = CreateBoundaryPoints(duplicate);
...
void StaticVoronoi::Build(vector<Vector3D> const & /*points*/)
{
}

void StaticVoronoi::Init(vector<Vector3D> const & points)
{
	assert(points.size() > 0);
	// Clear data
	PointTetras_.clear();
...
void StaticVoronoi::BuildVoronoi(void)
{
...
double StaticVoronoi::GetRadius(std::size_t index)
{
	if (R_[index] < 0)
...
double StaticVoronoi::GetMaxRadius(std::size_t index)
{
...
void  StaticVoronoi::FindIntersectionsSingle(vector<Face> const& box, std::size_t point, Sphere &sphere,
...
vector<std::size_t>  StaticVoronoi::FindIntersectionsRecursive(Tessellation3D const& tproc, std::size_t rank, std::size_t point,
...
void StaticVoronoi::GetPointToCheck(std::size_t point, vector<bool> const& checked, vector<std::size_t> &res)
{
	res.clear();
	std::size_t ntetra = PointTetras_[point].size();
...
std::size_t StaticVoronoi::GetFirstPointToCheck(void)const
{
...
#ifdef RICH_MPI
vector<std::pair<std::size_t, std::size_t> > StaticVoronoi::FindIntersections(Tessellation3D const& tproc, bool recursive)
{
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
vector<std::pair<std::size_t, std::size_t> > StaticVoronoi::SerialFindIntersections()
{
	if (Norg_ < 50)
	{
		vector<std::pair<std::size_t, std::size_t> > res;
...
double StaticVoronoi::CalcTetraRadiusCenter(std::size_t index)
{
...
Vector3D StaticVoronoi::GetTetraCM(boost::array<Vector3D, 4> const& points)const
{
	Vector3D res;
	for (std::size_t i = 0; i < 4; ++i)
		res += points[i];
...
double StaticVoronoi::GetTetraVolume(boost::array<Vector3D, 4> const& points)const
{
...
void StaticVoronoi::CalcCellCMVolume(std::size_t index)
{
	volume_[index] = 0;
	CM_[index] = Vector3D();
...
void StaticVoronoi::output(std::string const& filename)const
{
...
std::size_t StaticVoronoi::GetPointNo(void) const
{
	return Norg_;
}

Vector3D StaticVoronoi::GetMeshPoint(std::size_t index) const
{
	return del_.points_[index];
}

double StaticVoronoi::GetArea(std::size_t index) const
{
	return area_[index];
}

Vector3D const& StaticVoronoi::GetCellCM(std::size_t index) const
{
	return CM_[index];
}

std::size_t StaticVoronoi::GetTotalFacesNumber(void) const
{
	return FaceNeighbors_.size();
}

double StaticVoronoi::GetWidth(std::size_t index) const
{
	return pow(3 * volume_[index] * 0.25 / M_PI, 0.333333333);
}

double StaticVoronoi::GetVolume(std::size_t index) const
{
	return volume_[index];
}

vector<std::size_t>const& StaticVoronoi::GetCellFaces(std::size_t index) const
{
	return FacesInCell_[index];
}

vector<Vector3D>& StaticVoronoi::GetMeshPoints(void)
{
	return del_.points_;
}

vector<std::size_t> StaticVoronoi::GetNeighbors(std::size_t index)const
{
...
void StaticVoronoi::GetNeighbors(size_t index, vector<size_t> &res)const
{
	std::size_t N = FacesInCell_[index].size();
	res.resize(N);
	for (std::size_t i = 0; i < N; ++i)
...
Tessellation3D* StaticVoronoi::clone(void) const
{
	return new StaticVoronoi(*this);
}

bool StaticVoronoi::NearBoundary(std::size_t index) const
{
	std::size_t N = FacesInCell_[index].size();
	for (std::size_t i = 0; i < N; ++i)
	{
...
bool StaticVoronoi::IsPointOutsideBox(size_t index)const
{
	return !PointInDomain(ll_, ur_, del_.points_[index]);
}

bool StaticVoronoi::BoundaryFace(std::size_t index) const
{
...
vector<vector<std::size_t> >& StaticVoronoi::GetDuplicatedPoints(void)
{
	return duplicated_points_;
}

vector<vector<std::size_t> >const& StaticVoronoi::GetDuplicatedPoints(void)const
{
	return duplicated_points_;
}

std::size_t StaticVoronoi::GetTotalPointNumber(void)const
{
	return del_.points_.size();
}

vector<Vector3D>& StaticVoronoi::GetAllCM(void)
{
	return CM_;
}

void StaticVoronoi::GetNeighborNeighbors(vector<std::size_t> &result, std::size_t point)const
{
	result.clear();
	result.reserve(70);
...
vector<vector<size_t> > & StaticVoronoi::GetAllPointsInFace(void)
{
	return PointsInFace_;
}

size_t& StaticVoronoi::GetPointNo(void)
{
	return Norg_;
}

std::vector<std::pair<size_t, size_t> >& StaticVoronoi::GetAllFaceNeighbors(void)
{
	return FaceNeighbors_;
}

vector<double>& StaticVoronoi::GetAllVolumes(void)
{
	return volume_;
}

Vector3D StaticVoronoi::Normal(std::size_t faceindex)const
{
	return del_.points_[FaceNeighbors_[faceindex].second] - del_.points_[FaceNeighbors_[faceindex].first];
}

bool StaticVoronoi::IsGhostPoint(std::size_t index)const
{
	return index >= Norg_;
}

Vector3D StaticVoronoi::FaceCM(std::size_t index)const
{
...
Vector3D StaticVoronoi::CalcFaceVelocity(std::size_t index, Vector3D const& v0, Vector3D const& v1)const
{
...
vector<double>& StaticVoronoi::GetAllArea(void)
{
	return area_;
}

vector<vector<size_t> >& StaticVoronoi::GetAllCellFaces(void)
{
	return FacesInCell_;
}

vector<Vector3D>& StaticVoronoi::GetFacePoints(void)
{
	return tetra_centers_;
}

vector<Vector3D>const& StaticVoronoi::GetFacePoints(void)const
{
	return tetra_centers_;
}


vector<std::size_t>const& StaticVoronoi::GetPointsInFace(std::size_t index) const
{
	return PointsInFace_[index];
}

std::pair<std::size_t, std::size_t> StaticVoronoi::GetFaceNeighbors(std::size_t face_index)const
{
	return std::pair<std::size_t, std::size_t>(FaceNeighbors_[face_index]);
}

vector<int> StaticVoronoi::GetDuplicatedProcs(void)const
{
	return duplicatedprocs_;
}

vector<int> StaticVoronoi::GetSentProcs(void)const
{
	return sentprocs_;
}

vector<vector<std::size_t> > const& StaticVoronoi::GetSentPoints(void)const
{
	return sentpoints_;
}

vector<std::size_t> const& StaticVoronoi::GetSelfIndex(void) const
{
	return self_index_;
}
