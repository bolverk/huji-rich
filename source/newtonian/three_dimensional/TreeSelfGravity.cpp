	...
	vector<Vector3D>& acc) const
{
	...
	// Setup constants
	r->integrator = REB_INTEGRATOR_EULER;
	...
	reb_configure_box(r, boxsize, nx_, ny_, nz_);
	...
	else
	{
		// recv the acc
		temp.resize(3 * Npart);
		MPI_Recv(&temp[0], static_cast<size_t>(3 * Npart), MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
