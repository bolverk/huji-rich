...
namespace
{
		...
		bool first = true)
	{
		...
		if ((usefirst&&first) || (!first&&!usefirst))
		{
			...
			for (size_t i = 0; i<edges.size(); ++i)
			{
				Edge const& etemp = tessnew.GetEdge(edges[i]);
	...
	{
		int e1 = -1, e2 = -1;
		vector<int> edgesnew = tessnew.GetCellEdges(cell_index);
		for (size_t i = 0; i<edgesnew.size(); ++i)
		{
			Edge const& etemp = tessnew.GetEdge(edgesnew[i]);
			if (tessnew.GetOriginalIndex(etemp.neighbors.first) == n0 ||
				tessnew.GetOriginalIndex(etemp.neighbors.second) == n1 ||
				tessnew.GetOriginalIndex(etemp.neighbors.second) == n0 ||
				tessnew.GetOriginalIndex(etemp.neighbors.first) == n1)
		...
		if (e1 != -1 && e2 != -1)
		{
			Edge const& e1temp = tessnew.GetEdge(e1);
			Edge const& e2temp = tessnew.GetEdge(e2);
			vector<double> dist_temp(4);
			dist_temp[0] = abs(e1temp.vertices.first - e2temp.vertices.first);
			dist_temp[1] = abs(e1temp.vertices.first - e2temp.vertices.second);
			dist_temp[2] = abs(e1temp.vertices.second - e2temp.vertices.first);
			dist_temp[3] = abs(e1temp.vertices.second - e2temp.vertices.second);
			vector<size_t> temp_ind = sort_index(dist_temp);
			const double eps2 = 1e-6;
			const double R = tessnew.GetEdge(old_edge).GetLength();
			if (dist_temp[temp_ind[0]] > R*eps2)
				return false;
			switch (temp_ind[0])
			{
			case 0:
				res = e1temp.vertices.first;
				break;
			case 1:
				res = e1temp.vertices.first;
				break;
			case 2:
				res = e1temp.vertices.second;
				break;
			case 3:
				res = e1temp.vertices.second;
				break;
		...
		{
			Edge const& e1temp = tessnew.GetEdge(e1);
			Edge const& e_old = tessold.GetEdge(old_edge);
			double minimum1 = min(abs(e1temp.vertices.first + added - e_old.vertices.first),
				abs(e1temp.vertices.first + added - e_old.vertices.second));
			double minimum2 = min(abs(e1temp.vertices.second + added - e_old.vertices.first),
				abs(e1temp.vertices.second + added - e_old.vertices.second));
		...
		if (e2 != -1)
		{
			Edge const& e2temp = tessnew.GetEdge(e2);
			Edge const& e_old = tessold.GetEdge(old_edge);
			double minimum1 = min(abs(e2temp.vertices.first + added - e_old.vertices.first),
				abs(e2temp.vertices.first + added - e_old.vertices.second));
			double minimum2 = min(abs(e2temp.vertices.second + added - e_old.vertices.first),
				abs(e2temp.vertices.second + added - e_old.vertices.second));
			if (minimum1 < minimum2)
				res = e2temp.vertices.first;
			else
				res = e2temp.vertices.second;
			return true;
		}
		else
			return false;
	...
	Vector2D FindVerticeAppearHelper(Tessellation const& tessold, Tessellation const& tessnew, int n0, int n1, int cell_index)
	{
		int e0 = -1;
		int e1 = -1;
		vector<int> const& edges = tessold.GetCellEdges(cell_index);
		for (size_t i = 0; i < edges.size(); ++i)
		{
			Edge const& etemp = tessold.GetEdge(edges[i]);
			if (tessold.GetOriginalIndex(etemp.neighbors.first) == n0 ||
				tessold.GetOriginalIndex(etemp.neighbors.first) == n1 ||
				tessold.GetOriginalIndex(etemp.neighbors.second) == n0 ||
				tessold.GetOriginalIndex(etemp.neighbors.second) == n1)
			{
				if (e0 == -1)
					e0 = edges[i];
				else
		...
		if (e1 >= 0)
		{
			Edge const& e1temp = tessold.GetEdge(e0);
			Edge const& e2temp = tessold.GetEdge(e1);
			vector<double> dist_temp(4);
			dist_temp[0] = abs(e1temp.vertices.first - e2temp.vertices.first);
			dist_temp[1] = abs(e1temp.vertices.first - e2temp.vertices.second);
			dist_temp[2] = abs(e1temp.vertices.second - e2temp.vertices.first);
			...
			{
			case 0:
				return e1temp.vertices.first;
			case 1:
				return e1temp.vertices.first;
			case 2:
				return e1temp.vertices.second;
			case 3:
				return e1temp.vertices.second;
			}
		}
		UniversalError eo("Couldn't find edge in FindVerticeAppearHelper");
		throw eo;
	}

	bool FindVerticeAppear(Tessellation const& tessold, Tessellation const& tessnew, int n0, int n1, int cell_index,
		int new_edge, Vector2D const& added, Vector2D &res,int &old_edge)
	{
		int e1 = -1, e2 = -1;
		old_edge = -1;
		vector<int> edgesnew = tessold.GetCellEdges(cell_index);
		for (size_t i = 0; i<edgesnew.size(); ++i)
		{
			Edge const& etemp = tessold.GetEdge(edgesnew[i]);
			if (tessold.GetOriginalIndex(etemp.neighbors.first) == n0 ||
				tessold.GetOriginalIndex(etemp.neighbors.second) == n1 ||
				tessold.GetOriginalIndex(etemp.neighbors.second) == n0 ||
				tessold.GetOriginalIndex(etemp.neighbors.first) == n1)
		...
		{
			const double eps2 = 1e-6;
			const double R = tessnew.GetEdge(new_edge).GetLength();
			Edge const& e1temp = tessold.GetEdge(e1);
			Edge const& e2temp = tessold.GetEdge(e2);
			vector<double> dist_temp(4);
			dist_temp[0] = abs(e1temp.vertices.first - e2temp.vertices.first);
			dist_temp[1] = abs(e1temp.vertices.first - e2temp.vertices.second);
			dist_temp[2] = abs(e1temp.vertices.second - e2temp.vertices.first);
			dist_temp[3] = abs(e1temp.vertices.second - e2temp.vertices.second);
			vector<size_t> temp_ind = sort_index(dist_temp);
			// Is there an old edge that should be connected to the new edge?
			if (dist_temp[temp_ind[0]] > R*eps2)
			{
				for (size_t i = 0; i < edgesnew.size(); ++i)
				{
					Edge const& etemp = tessold.GetEdge(edgesnew[i]);
					if ((abs(etemp.vertices.first - e1temp.vertices.first) < R*eps2 &&
						abs(etemp.vertices.second - e2temp.vertices.first) < R*eps2) ||
						(abs(etemp.vertices.first - e1temp.vertices.second) < R*eps2 &&
						abs(etemp.vertices.second - e2temp.vertices.first) < R*eps2) ||
						(abs(etemp.vertices.first - e1temp.vertices.first) < R*eps2 &&
						abs(etemp.vertices.second - e2temp.vertices.second) < R*eps2) ||
						(abs(etemp.vertices.first - e1temp.vertices.second) < R*eps2 &&
						abs(etemp.vertices.second - e2temp.vertices.second) < R*eps2))
					{
						old_edge = edgesnew[i];
						Edge const& etemp2 = tessnew.GetEdge(new_edge);
						const int other_index = tessold.GetOriginalIndex(etemp2.neighbors.first)==cell_index ?
							tessold.GetOriginalIndex(etemp2.neighbors.second) :
							tessold.GetOriginalIndex(etemp2.neighbors.first);
						res = FindVerticeAppearHelper(tessold, tessnew, n0, n1, other_index);
						return true;
					}
				}
			}
			// Find the old vertice to connect to the old edge
			switch (temp_ind[0])
			{
			case 0:
				res = e1temp.vertices.first;
		...
		if (e1 != -1)
		{
			...
			if (old_edge < 0)
			{
				Edge const& e_old = tessnew.GetEdge(new_edge);
				double minimum1 = min(abs(e1temp.vertices.first + added - e_old.vertices.first),
					abs(e1temp.vertices.first + added - e_old.vertices.second));
				double minimum2 = min(abs(e1temp.vertices.second + added - e_old.vertices.first),
					abs(e1temp.vertices.second + added - e_old.vertices.second));
	...
	Vector2D GetNeighborDiff(Tessellation const& tess, int cell_index, int other)
	{
		...
		for (size_t i = 0; i<edges.size(); ++i)
		{
			...
			if (tess.GetOriginalIndex(edge.neighbors.first) == other)
				return tess.GetMeshPoint(edge.neighbors.first) - tess.GetMeshPoint(other);
			if (tess.GetOriginalIndex(edge.neighbors.second) == other)
				return tess.GetMeshPoint(edge.neighbors.second) - tess.GetMeshPoint(other);
		}
		return Vector2D(0, 0);
	}

	int FindIndexGivenInEdge(Tessellation const& tess, int n0, int cell_index)
	{
		vector<int> const& edges = tess.GetCellEdges(cell_index);
		for (size_t i = 0; edges.size(); ++i)
		{
			Edge const& edge = tess.GetEdge(edges[i]);
			if (tess.GetOriginalIndex(edge.neighbors.first) == n0)
				return edge.neighbors.first;
			if (tess.GetOriginalIndex(edge.neighbors.second) == n0)
				return edge.neighbors.second;
		}
		return -1;
	}

	void CalcNewTrianlgeArea(Vector2D const& p0,Vector2D const& p1,Vector2D const& real_p_new,Edge const& edge,
		Tessellation const& tessnew,int cell_index,
		std::pair<Vector2D, Vector2D> &res, std::pair<int, int> &resneigh)
	{
	Edge olde0;
	olde0.vertices.first = p0;
	olde0.vertices.second = p1;
	Vector2D other = (edge.neighbors.first == cell_index) ? tessnew.GetMeshPoint(
		edge.neighbors.second) : tessnew.GetMeshPoint(edge.neighbors.first);

	if (RightHand(edge, real_p_new))
	{
		TripleConstRef<Vector2D> points(edge.vertices.second, edge.vertices.first, olde0.vertices.first);
		res.first.x = 0.5*orient2d(points);
	}
	else
	{
	...
	if (!RightHand(edge, other))
	{
		TripleConstRef<Vector2D> points(edge.vertices.second, edge.vertices.first, olde0.vertices.second);
		res.second.x = 0.5*orient2d(points);
	}
	else
	{
		TripleConstRef<Vector2D> points(edge.vertices.second, edge.vertices.first, olde0.vertices.second);
...
//		resneigh.first = (res.first.x + res.second.x>0) ? n0other : n1other;
	//	resneigh.second = (resneigh.first == n0other) ? n1other : n0other;
	}

	void TrianlgeAppear(Edge const& edge, Tessellation const& tessold,
		Tessellation const& tessnew, HydroBoundaryConditions	const& hbc, std::pair<Vector2D, Vector2D> &res, std::pair<int, int> &resneigh,
		int cell_index, int other_index, vector<Vector2D> const& pointvelocity, double dx, double dy,
		int edge_index)
	{
		res.first.Set(0, 0);
		res.second.Set(0, 0);
		...
		resneigh.second = -1;
		int n0other, n1other;
		GetAdjacentNeigh(tessnew, edge, cell_index, Vector2D(0, 0), n0other, n1other);
		Vector2D real_p_new = tessnew.GetMeshPoint(cell_index);
		Vector2D real_p = tessold.GetMeshPoint(cell_index);
		Vector2D diff = PeriodDiff(pointvelocity[cell_index], dx, dy, real_p, real_p_new);
		diff.Set(-diff.x, -diff.y);

		Vector2D real_p_new2 = tessnew.GetMeshPoint(other_index);
		Vector2D real_p2 = tessold.GetMeshPoint(other_index);
		Vector2D diff2 = PeriodDiff(pointvelocity[other_index], dx, dy, real_p2, real_p_new2);
		diff2.Set(-diff2.x, -diff2.y);
		diff2 += (edge.neighbors.first == cell_index) ? tessnew.GetMeshPoint(
			edge.neighbors.second) - real_p_new2 : tessnew.GetMeshPoint(edge.neighbors.first) -
			real_p_new2;

		Vector2D p0, p1;
		int old_edge;
		if (!FindVerticeAppear(tessold, tessnew, n0other, n1other, cell_index, edge_index, -1*diff, p0,old_edge))
			return;
		if (old_edge >= 0)
		{
			Edge etemp = tessnew.GetEdge(edge_index);
			etemp.vertices.first += diff;
			etemp.vertices.second += diff;
			double area=EdgesArea(tessold.GetEdge(old_edge), etemp, real_p, real_p_new-diff);
			res.first.x = -area;
			resneigh.first = area<0 ? other_index : cell_index;
			return;
		}
		if (!FindVerticeAppear(tessold, tessnew, n0other, n1other, other_index, edge_index, -1*diff, p1, old_edge))
			return;
		if (old_edge >= 0)
		{
			Edge etemp = tessnew.GetEdge(edge_index);
		...
		old_edge = GetMutualEdge(tessold, n0other, n1other, true);

		if (old_edge >= 0)
		{
			Edge olde0 = tessold.GetEdge(old_edge);
			//int eindex=FindIndexGivenInEdge(tessold,n0other,cell_index);
			//Vector2D old_diff=tessold.GetMeshPoint(eindex)-tessold.GetMeshPoint(n0other);
			olde0.vertices.first = p0 + diff;
			olde0.vertices.second = p1 + diff2;
			Vector2D other = (edge.neighbors.first == cell_index) ? tessnew.GetMeshPoint(
				edge.neighbors.second) : tessnew.GetMeshPoint(edge.neighbors.first);

			if (DoEdgesIntersect(edge, olde0))
				Get4Area(tessold, tessnew, old_edge, edge, res, resneigh);
			else
			{
				if (RightHand(edge, real_p_new))
				{
...
//			checked_mid[mid_index] = 1;
	//		if (hbc.IsBoundary(tessmid.GetEdge(mid_index), tessmid))
		...
		if (other_index == -1)
			continue;
		Edge edge2 = tessnew.GetEdge(new_index);
		if (outer.GetBoundaryType() == Periodic)
		{
			edge2.vertices.first += added;
			edge2.vertices.second += added;
			real_p_new += added;
		}
		double area = EdgesArea(edge, edge2, real_p, real_p_new);
	...
	HydroBoundaryConditions const& hbc)
{
	...
	for (size_t i = 0; i < checked_new.size(); ++i)
	{
		if (checked_new[i] == 1)
			continue;
		Edge const& edge = tessnew.GetEdge(i);
		int n0, n1;
