...
namespace
{
	vector<unsigned long long> SortAllPoints(size_t Ncor, size_t Nproc, vector<size_t> &Hindeces,Tessellation3D const& tess)
	{
		...
		if (counter > static_cast<size_t>(Npart*segfraction))
	...
	{
		vector<Vector3D> res(Nproc);
		vector<int> NperProc(Nproc, 0);
		...
		for (size_t i = 0; i < Ncor; ++i)
		{
			size_t index = static_cast<size_t>(std::lower_bound(Hxcor.begin(), Hxcor.end(), static_cast<unsigned long long>(H_indeces[i]))
				- Hxcor.begin());
			NperProc[index]++;
			res[index] += tess.GetMeshPoint(i);
		}
#ifdef RICH_MPI
		vector<int> NperProcRecv(Nproc, 0);
		MPI_Allreduce(&NperProc[0], &NperProcRecv[0], static_cast<int>(Nproc), MPI_INT, MPI_SUM, MPI_COMM_WORLD);
		NperProc = NperProcRecv;
		vector<double> sendtemp(Nproc * 3, 0), recvtemp(Nproc * 3, 0);
		for (size_t i = 0; i < Nproc; ++i)
		{
			sendtemp[3 * i] = res[i].x;
			sendtemp[3 * i + 1] = res[i].y;
			sendtemp[3 * i + 2] = res[i].z;
		}
		MPI_Allreduce(&sendtemp[0], &recvtemp[0], static_cast<int>(3*Nproc), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		for (size_t i = 0; i < Nproc; ++i)
		{
			res[i].x = recvtemp[3 * i];
			res[i].y = recvtemp[3 * i + 1];
			res[i].z = recvtemp[3 * i + 2];
		}
#endif
		for (size_t i = 0; i < Nproc; ++i)
			res[i] *= (1.0) / NperProc[i];
		return res;
	}
}
...
vector<Vector3D> HilbertProcPositions(Tessellation3D const & tess)
{
	...
	vector<Vector3D> res = GetNewProcPoints(Hindex, tess, H_indeces, Nproc);
	return res;
}
