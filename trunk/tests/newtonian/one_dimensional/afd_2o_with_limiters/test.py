...
def main(graphic_flag=False):
    ...
    import numpy
    import imp
    afd = imp.load_source('afd','../../analytic/afd.py')
    import argparse
    ...
    final_1o = afd.first_order_time_advance(init_parsed, time)
    ...
    if graphic_flag:
        ...
        for i, field in enumerate(['density',
            ...
            pylab.plot(initial['grid'],
                       init_parsed['pert'][field],'b')
            pylab.plot(initial['grid'],
                       final_1o[field],'g')
            pylab.plot(initial['grid'],
                       final_2o[field],'r')
            pylab.plot(initial['grid'], final_exact[field], 'c')
    ...
    return abs(l1_list['density'])<2e-5 and \
        abs(l1_list['pressure'])<2e-5 and \
        abs(l1_list['velocity'])<2e-5
