...
def main():
    ...
    v_init = initial['x_velocity']

    final = h5py.File('final.h5')
    time = numpy.array(final['time'])[0]
    x_numeric = final['grid']
    d_numeric = final['density']
    p_numeric = final['pressure']
    v_numeric = final['x_velocity']
    ...
    test_name = 'L1'
    min_x_numeric = numpy.min(x_numeric)
    max_x_numeric = numpy.max(x_numeric)
    xc = [x for x in x_exact if x>min_x_numeric and x<max_x_numeric]
