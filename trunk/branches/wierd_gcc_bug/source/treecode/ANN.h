...
class DLL_API ANNbruteForce: public ANNpointSet {
	...
	ANNbruteForce(const ANNbruteForce& origin);
	ANNbruteForce& operator=(const ANNbruteForce& origin);
public:
...
class DLL_API ANNkd_tree: public ANNpointSet {
  private:
  ANNkd_tree(const ANNkd_tree& origin);
  ANNkd_tree& operator=(const ANNkd_tree& origin);
protected:
	int				dim;				// dimension of space
...
public:
	double GetTimeStep(int index,std::vector< std::vector<double> >const& vel,
		double dt,std::vector<double> const& c);

	void SetMass(std::vector<double> const& masses);

	void SetVmax(std::vector<double> const& v);

	void SetCmax(std::vector<double> const& c);

	void SetH(std::vector<double> const& Hvec);

	std::vector<double> GetAcc(std::vector<double> const& qpoint,
		double openAngle);

	double GetPot(std::vector<double> const& qpoint,
		double openAngle,double soft);
	...
	/*vector<double> GetAcc(ANNkd_split box,vector<double> const& masses,
		ANNpoint q,double openning);*/
};								
