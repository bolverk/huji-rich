...
public:
		...
		double /*time*/)const
	{
		...
		{
			vector<int> neigh=tess->GetNeighbors(res[i]);
			bool good=true;
			if(find(bad_neigh.begin(),bad_neigh.end(),res[i])!=
				bad_neigh.end())
				good=false;
			else
			{
				...
				{
					if(binary_search(res.begin(),res.end(),neigh[j]))
					{
						double dx2=1.0/(1-tess->GetMeshPoint(neigh[j]).x);
						if(merit[i]<dx2)
						{
							good=false;
							break;
						...
						if(merit[i]==dx2)
						{
							if(!binary_search(bad_neigh.begin(),bad_neigh.end(),neigh[j]))
...
int main(void)
{
  ...
  VoronoiMesh tess;
  LinearGauss interp(outer,&hbc,true,false,0.1,0.4);
  //PCM2D interp;
  EOSConsistent pcm2d(interp,eos);
  ...
  Lagrangian bpm;
  RoundCells pointmotion(bpm,0.65,0.05,np,&outer);
