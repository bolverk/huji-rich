	...
	HydroBoundaryConditions const* hbc,double time)
{
	...
	double dt;
	if(!tessellation->NearBoundary(0))
		dt = TimeStepForCell(cells[0],tessellation->GetWidth(0),face_vel);
	else
		dt = TimeStepForCellBoundary(cells[0],cells,tessellation->GetWidth(0),
		face_vel,tessellation,hbc,0,time);
	double dt_temp;
	for(int i = 1;i<tessellation->GetPointNo(); i++)
	{
...
void UpdatePrimitives
	...
	bool densityfloor,double densitymin,double pressuremin)
{
	Conserved temp;
	for(int i=0;i < (int)cells.size(); i++){
		try
			...
			if(CellsEvolve[i]==0)
			{
				...
				if(densityfloor)
				{
				  if(temp.Mass<densitymin)
					...
					double p=eos.de2p(temp.Mass,(temp.Energy-0.5*ScalarProd(
						temp.Momentum,temp.Momentum)/temp.Mass)/temp.Mass);
					if(p<pressuremin)
	...
	vector<vector<double> > const& tracers)
{
	...
	interpolation->Prepare(tessellation,cells,dt,
		is_normal_evolution(CellsEvolve),time);
...
double TimeAdvance2mid
	...
	vector<vector<double> > &tracers,ScalarInterpolation *sinterp,
	...
	double pressuremin,bool EntropyCalc)
{
	//save tessellation
	/*
	Tessellation *tess_temp;
	tess_temp=tess->clone();
	*/
	...
	double dt = cfl*CalcTimeStep
		(tess_temp,cells, edge_velocities,hbc,time);
	...
	if(traceflag)
	{
		...
		trace_change = CalcTraceChange(tracers,tess_temp,fluxes,0.5*dt,hbc,
			sinterp,time);
	...
	{
		vector<vector<double> > trace_change;
		trace_change = CalcTraceChange(tracers,tess_temp,fluxes,dt,hbc,sinterp,
			time);
		tracers=old_trace;
...
vector<vector<double> > CalcTraceChange(vector<vector<double> > const& old_trace,
	Tessellation const* tess,vector<Conserved> const& fluxes,double dt,
	HydroBoundaryConditions const* hbc,ScalarInterpolation *interp,
	double time)
{
	if(interp==0)
		throw UniversalError("Error in tracer, no interpolation given");
	...
	if(n>0)
		...
		return res;
	res.resize(n);
	interp->Prepare(tess,old_trace,dt,time);
	double dm;
		...
		for(int k=0;k<Nedges;++k)
		{
			...
			if(dm>0)
			{
				if(i==n1)
				{
					if(hbc->IsBoundary(edge,tess))
					{
						temp=hbc->CalcTracerFlux(tess,old_trace,dm,edge,i,dt,time,interp);
						transform(res[i].begin(),res[i].end(),temp.begin(),res[i].begin(),
							plus<double>());
					}
					else
					{
						temp=interp->Interpolate(tess,old_trace,dt,edge,0,SInBulk);
			...
			else
			{
				if(i==n1)
				{
					...
					{
						temp=interp->Interpolate(tess,old_trace,dt,edge,1,SInBulk);
						transform(temp.begin(),temp.end(),temp.begin(),
							bind1st(multiplies<double>(),dm*dt*edge.GetLength()));
						transform(res[i].begin(),res[i].end(),temp.begin(),res[i].begin(),
							plus<double>());
