...
void hdsim::setTracerInterpolation(ScalarInterpolation* tracer_interp)
{
	tracer_interp_ = tracer_interp;
}
...
void hdsim::TimeAdvance(void)
{
	...
	if(tracer_flag_)
	{
		...
		trace_change = CalcTraceChange(tracer_,_tessellation,_fluxes,dt,_hbc,tracer_interp_,_time);
...
void hdsim::TimeAdvance2Mid(void)
{
  const double dt=TimeAdvance2mid
    (_tessellation,_cells,_pointmotion,
     ...
     CellsEvolve,tracer_,tracer_interp_,_dt_external,tracer_flag_,
