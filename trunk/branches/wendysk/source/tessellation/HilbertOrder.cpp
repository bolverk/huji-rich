...
namespace
{
	...
	public:
		...
		AllLocalData(vector<int> *vin,int iN,int _n,int _pow2);
...
AllMyParameters::AllMyParameters(vector<int>* vin):
v(vin) {}

AllMyParameters::AllMyParameters(AllMyParameters const& other):
v(other.v) {}

AllLocalData::AllLocalData(vector<int>* vin, int iN, int inn, int ipow2):
IndexTable(vin), N(iN), n(inn), pow2(ipow2) {}

AllLocalData::AllLocalData(AllLocalData const& other):
IndexTable(other.IndexTable),
	N(other.N),
	n(other.n),
	pow2(other.pow2) {}

Context::Context(AllMyParameters const& par,
	AllLocalData const& local_data,
	int i):
