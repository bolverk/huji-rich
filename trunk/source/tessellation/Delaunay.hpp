...
private:
  ...
  Vector2D GetBoundaryPoint(int index,Sides side,Edge const& edge);
...
public:
  ...
  */
  ...
  /*! \brief Builds the boundary points (no corners)
    \param edges The edges of the domain
    \param toduplicate The list of points to duplicate sorted coressponding to the reflecting edge
  */
  void DoBoundary(vector<Edge> const& edges,vector<vector<int> > const& toduplicate);
  /*! \brief Builds the corners of the boundary
    \param edges The edges of the domain
    \param toduplicate The list of points to duplicate sorted coressponding to the reflecting edge
  */
  void DoCorners(vector<Edge> const& edges,vector<vector<int> > const& toduplicate);

  /*!
  \brief Adds a point to the cor vector. Used in periodic boundaries with AMR.
  \param vec The point to add.
  \param index The index of the original point
  */
  void AddAditionalPoint(Vector2D const& vec,int index);
   /*!
