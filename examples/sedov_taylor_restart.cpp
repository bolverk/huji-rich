...
int main(void)
{
  ...
  // Main loop
  {
    ...
    simulation2d::main_loop(sim,
			    ...
			    &diag);
    write_snapshot_to_hdf5(sim,"for_restart.h5");
  }

  // Use output to restart a new simulation
  ResetDump dump;
  read_hdf5_snapshot(dump,"for_restart.h5",&eos);

  // Set up the simulation
  hdsim sim2(dump,tess,interpolation,eos,rs,pointmotion,force,outer,hbc);

  // Main loop
  const double tend2 = 0.1;
  sim2.SetEndTime(tend2);

  {
    SafeTimeTermination term_cond(tend2,1e6);
    ProgressReport diag1(25);
    ConsecutiveSnapshots diag2(0.01);
    MultipleDiagnostics diag;
    diag.diag_list.push_back(&diag1);
    diag.diag_list.push_back(&diag2);
    simulation2d::main_loop(sim2,
			    term_cond,
			    &hdsim::TimeAdvance2Mid,
			    &diag);
  }
